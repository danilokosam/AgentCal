...
export function AppointmentBlock({ appointment }: Props) {
  const timeRange = `${formatTime(appointment.start_time)} – ${formatTime(appointment.end_time)}`;

  return (
    <Tooltip>
      <TooltipTrigger
        className="absolute inset-x-0.5 rounded-md bg-blue-600 text-white px-2 py-1 cursor-pointer hover:bg-blue-700 transition-colors overflow-hidden shadow-sm text-left"
        render={<div />}
      >
